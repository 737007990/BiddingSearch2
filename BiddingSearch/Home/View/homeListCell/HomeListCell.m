...
- (void)configCellWIthData:(id)data{
    if([data isKindOfClass:[HomeListCellModel class]]){
        HomeListCellModel *m=data;
        
        [_titleL setText:m.pname];
        NSAttributedString *attrStr = [[NSAttributedString alloc] initWithData:[m.tname dataUsingEncoding:NSUnicodeStringEncoding] options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType } documentAttributes:nil error:nil];
        [_contentL setAttributedText:attrStr];
        [_industryL setText:m.work];
        [_areaL setText:m.area];
        [_timeL setText:m.date];
