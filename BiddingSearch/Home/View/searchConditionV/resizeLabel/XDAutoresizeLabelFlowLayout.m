...
- (void)prepareLayout {
    ...
    for (int i = 0; i < section; i++) {
        ...
        if ((item&&_headVDefault)||(_alwaysShowHeadDefault&&_headVDefault)) {
        ...
        //当z自定义时间和金额以及最后m的按钮d时显示footer
        if ((i==2&&_show_nt_starttime)||i==section-1||(i==4&&_show_price)) {
            UICollectionViewLayoutAttributes *sectionAttfooter = [self layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionFooter atIndexPath:[NSIndexPath indexPathForItem:item-1 inSection:i]];
            [attributesArray addObject:sectionAttfooter];
        }
    }
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([elementKind isEqualToString:@"UICollectionElementKindSectionHeader"]) {
        ...
        if ((indexPath.section ==2&&_show_nt_starttime)||(indexPath.section==4&&_show_price)||indexPath.section==5) {
            orgin.totalY += itemHeight+lineSpace;
//             orgin.totalY += sectionHeight;
        }
          attri.frame = CGRectMake(0, orgin.totalY, self.collectionView.width, sectionHeight);
