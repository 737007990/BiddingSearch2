 ...
 *  @param action 回调方法
 */
- (void)addHeaderWithTarget:(id)target action:(SEL)action andFlag:(BOOL)haveLogo andTimeLabHidden:(BOOL)timeLabHidden
{
    [self addHeaderWithTarget:target action:action dateKey:nil andFlag:haveLogo andTimeLabHidden:timeLabHidden];
}

- (void)addHeaderWithTarget:(id)target action:(SEL)action dateKey :(NSString*)dateKey andFlag:(BOOL)haveLogo andTimeLabHidden:(BOOL)timeLabHidden
{
    // 1.创建新的header
    if (!self.header) {
        MHRefreshHeaderView *header = [MHRefreshHeaderView header];
        ...
        [self addSubview:header];
        self.header = header;
        self.header.flag = haveLogo;
        self.header.isTimeLabHidden = timeLabHidden;
    }
