...
{
    _flag = flag;
    __weak MHRefreshBaseView *wSelf = self;
    if (flag==YES)
    {
...
//        {
            if (!_cutomIma)
            {
                _cutomIma = [[UIImageView alloc]init];
                //        WithFrame:CGRectMake(0,0, activityView.bounds.size.width+20, activityView.bounds.size.width+20)
                [self addSubview:_cutomIma];
                [_cutomIma mas_makeConstraints:^(MASConstraintMaker *maker){
                    maker.centerY.equalTo(wSelf.mas_centerY);
                    maker.centerX.equalTo(wSelf.mas_centerX);
                    maker.size.mas_equalTo(CGSizeMake(40, 40));
                }];
                
                _cutomIma.center = _activityView.center;
                
                //        NSLog(@"============%f++++++++++++++%f",activityView.center.x,activityView.center.y);
                
                _cutomIma.layer.cornerRadius =20;
                _cutomIma.layer.masksToBounds = YES;
                _cutomIma.image = [UIImage imageNamed:@"2.pic_hd"];
                
                CABasicAnimation *baseAnimation = [CABasicAnimation animation];
                baseAnimation.repeatCount = NSIntegerMax;
                baseAnimation.duration = 0;
                baseAnimation.keyPath = @"transform";
                baseAnimation.fromValue = [NSValue valueWithCATransform3D:CATransform3DMakeRotation(M_PI*2, 0, 0, 1)];
                baseAnimation.toValue = [NSValue valueWithCATransform3D:CATransform3DMakeRotation(M_PI, 0, 0, 1)];
                [_cutomIma.layer addAnimation:baseAnimation forKey:@"transform"];
...
- (UIView *)customLoadingView
{
    ...
    if (!_customLoadingView) {
        ...
        // 添加自己想制作的动画等
        // ...
//        UIImageView *ima = [[UIImageView alloc]initWithFrame:self.bounds];
//        ima.image = [UIImage imageNamed:@"2.pic_hd"];
//        [_customLoadingView addSubview:ima];
